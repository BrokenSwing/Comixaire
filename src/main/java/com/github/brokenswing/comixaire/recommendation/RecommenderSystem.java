...
public class RecommenderSystem
{
    ...
    public double cosine(Client c1, Client c2) {
        double prod = 0;
        double abs1 = 0;
        double abs2 = 0;
        ...
        /**
         * Filter ratings by clients TODO: PROBLEM IS HERE!
         */
        ArrayList<Rating> c1_ratings = (ArrayList<Rating>) ratings.stream().filter(rating -> rating.getClient().equals(c1));
        ArrayList<Rating> c2_ratings = (ArrayList<Rating>) ratings.stream().filter(rating -> rating.getClient().equals(c2));
        ...
        for(Map.Entry<LibraryItem, Integer[]> entry : map.entrySet()) {
            ...
            if(notes[0] < 0){
                ...
                abs1 += Math.pow(notes[0], 2);
                abs2 += Math.pow(notes[1], 2);
                onlyOne = Math.min(Math.sqrt(notes[0] / notes[1]), Math.sqrt(notes[1] / notes[0]));
            }
        }
    ...
    public Set<Client> similarClients(Client client){
